      ...
      <div class="bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
          <div class="flex items-center justify-between mb-3">
            <span class="bg-yellow-100 text-yellow-800 text-xs font-semibold px-3 py-1 rounded-full">${e.duration}</span>
            <span class="text-2xl font-bold text-yellow-600">$${e.price}</span>
          </div>
          ...
          <button onclick="addToCart('${e.id}')" class="w-full bg-yellow-500 text-white font-semibold py-3 rounded-lg hover:bg-yellow-600 transition-colors">
            Add to Cart
          </button>
        </div>
      </div>
    `).join("")}function s(){document.getElementById("checkout-section").style.display="block",document.getElementById("checkout-section").scrollIntoView({behavior:"smooth"})}document.getElementById("checkout-btn")?.addEventListener("click",s);document.getElementById("checkout-form")?.addEventListener("submit",function(t){t.preventDefault(),alert("Thank you for your order! You will receive a confirmation email shortly.")});i();
